...
if __name__ == '__main__':
    ...
    parser.add_option("-d", "--InDir", type="string", help="Input Directory", dest="InDir", default="./sample/kap/")
    ...
    parser.add_option("-u", "--url", type="string", help="url", dest="url", default="ftp://ftp5.gwdg.de/pub/misc/openstreetmap/openseamap/charts/history/kap/")
    ...
    for filename in filenamelist:
        ...
        if(len(a.chartlist)):
            ...
            if kapfileworkaround is None:
                jsonres = Feature(geometry=Polygon([huell], properties=a.properties))
            else:

                try:
                    mapname = a.properties['name']
                except:
                    try:
                        mapname = a.properties['name:en']
                    except:
                        print("no mapname found")
                        assert(0)
                        pass

                try:
                    _format = a.properties['format']
                except:
                    _format = "KAP"
